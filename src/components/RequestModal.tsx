...
import { X, AlertTriangle, Plus, Trash2, FileCode2, Building2 } from 'lucide-react';
...
export const RequestModal: React.FC<Props> = ({
  ...
  // - thế chấp: nộp cho Ngân hàng giữ
  // - xuất bán / sang tên cho khách: giao cho Khách hàng giữ
  // - mượn: bộ phận nội bộ giữ tạm (có ngày hẹn trả)
  // - tách sổ / đổi sổ / thu hồi: nộp cơ quan chức năng
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-900">
                Tạo yêu cầu biến động ({selectedAssets.length} GCN đã chọn)
              </h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
              ...
              <span className="font-mono text-sm font-bold text-[#1E3A8A] bg-white px-2.5 py-1 rounded border border-blue-300 shadow-xs">
                {previewVoucherCode(
                  warehouses.find(w => w.id === targetWarehouseId) ||
                    warehouses.find(w => w.id === selectedAssets[0]?.warehouse_id) ||
                    warehouses[0],
                  selectedOpt.type,
                  selectedOpt.reason
                )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full rounded-lg border border-gray-300 p-2.5 text-xs bg-white focus:border-blue-500 focus:ring-blue-500 font-medium"
              >
          ...
          {selectedOpt?.reason === 'khác' && (
            <div className="pt-2">
              ...
              <textarea
                ...
                className="w-full rounded-lg border border-amber-300 bg-amber-50/50 p-2.5 text-xs focus:border-amber-500 focus:ring-amber-500"
              />
              {otherReasonDetail.trim().length > 0 && otherReasonDetail.trim().length < OTHER_REASON_MIN_LENGTH && (
                <p className="text-[11px] text-red-600 mt-1">
                  Vui lòng nhập ít nhất {OTHER_REASON_MIN_LENGTH} ký tự ({otherReasonDetail.trim().length}/
                  {OTHER_REASON_MIN_LENGTH}).
                </p>
              )}
            </div>
          )}
          ...
          {selectedOpt?.type === 'checkout' && (
            <div className="space-y-4 pt-2 border-t border-gray-100">
              <div className="grid grid-cols-2 gap-4">
                <div className={selectedOpt.reason !== 'mượn' ? 'col-span-2' : ''}>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-xs font-semibold text-gray-700">
                      Bộ phận sử dụng <span className="text-red-500">*</span>
                    </label>
                    <span className="text-[10px] text-blue-600 font-medium">(Tự động điền theo tài khoản)</span>
                  </div>
                  <input
                    required
                    type="text"
                    ...
                    placeholder="VD: Ban PTDA & Ban Đối Ngoại, Phòng Nguồn Vốn..."
                    className="w-full rounded-lg border border-gray-300 p-2.5 text-xs focus:border-blue-500 focus:ring-blue-500 font-medium"
                  />
                </div>
                {selectedOpt.reason === 'mượn' && (
                  <div>
                    <label className="block text-xs font-semibold text-gray-700 mb-1">
                      Ngày dự kiến trả <span className="text-red-500">*</span>
                    </label>
                    <input
                      required
                      type="date"
                      ...
                      className="w-full rounded-lg border border-gray-300 p-2.5 text-xs focus:border-blue-500 focus:ring-blue-500"
                    />
                  </div>
                )}
              </div>

              {/* CHỈ hiển thị trường "Kho nhận" khi lý do là Xuất chuyển kho / Chuyển nhượng */}
              {isTransferToWarehouse && (
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">
                    Kho nhận (nơi sổ sẽ được chuyển tới) <span className="text-red-500">*</span>
                  </label>
                  <select
                    required
                    value={targetWarehouseId}
                    onChange={e => setTargetWarehouseId(e.target.value)}
                    className="w-full rounded-lg border border-gray-300 p-2.5 text-xs bg-white focus:border-blue-500 focus:ring-blue-500"
                  >
                    <option value="">-- Chọn kho nhận --</option>
                    {warehouses.map(w => (
                      <option key={w.id} value={w.id}>
                        {w.name}
                        {w.is_central ? ' (Kho trung tâm)' : ''}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              {/* Thông báo thông tin đối với Xuất bán / Sang tên cho khách */}
              {isDirectCustomerExport && (
                <div className="p-3 bg-amber-50/90 border border-amber-200 rounded-lg text-xs text-amber-900 flex items-start gap-2.5">
                  ...
                  <div>
                    <div className="font-bold">Xuất bàn giao trực tiếp cho khách hàng / đối tác:</div>
                    <div className="text-[11px] text-amber-800 mt-0.5">
                      GCN được xuất khỏi hệ thống lưu kho để bàn giao cho khách hàng (không chuyển sang kho nội bộ).
                    </div>
                  </div>
                </div>
              )}
              ...
              {selectedOpt.reason === 'chuyển nhượng' && userRole === 'capital_dept' && (
                <div className="pt-2">
                  <label className="flex items-center space-x-2">
                    ...
                    />
                    <span className="text-xs font-medium text-gray-700">Đồng thời xuất thế chấp</span>
                  </label>
                </div>
              )}
            </div>
          ...
          {selectedOpt?.type === 'checkin' && (
            <div className="space-y-4 pt-2 border-t border-gray-100">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">
                    Ngày nhập thực tế <span className="text-red-500">*</span>
                  </label>
                  <input
                    required
                    type="date"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">
                    Kho nhập sổ <span className="text-red-500">*</span>
                  </label>
                  <select
                  ...
                  <div className="grid grid-cols-2 gap-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full rounded-md border border-gray-300 p-2 text-xs focus:border-blue-500 focus:ring-blue-500 bg-white"
                      >
                        <option value="">-- Chọn pháp nhân --</option>
                        {investorEntities.map(e => (
                          <option key={e.id} value={e.id}>
                            {e.name} ({e.company_code})
                          </option>
                        ))}
                      </select>
                    </div>
          ...
          {/* Form phụ theo lý do Thế chấp */}
          {(selectedOpt?.reason === 'thế chấp' || (selectedOpt?.reason === 'chuyển nhượng' && concurrentMortgage)) && (
            <div className="grid grid-cols-2 gap-4 bg-blue-50/50 p-3.5 rounded-lg border border-blue-100">
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">Ngân hàng thế chấp 1 *</label>
                <input
                  required
                  type="text"
                  value={bank}
                  onChange={e => setBank(e.target.value)}
                  placeholder="VD: BIDV - CN TP.HCM"
                  className="w-full rounded-md border border-gray-300 p-2 text-xs bg-white focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">Đơn vị vay 1 *</label>
                <input
                  required
                  type="text"
                  value={borrower}
                  onChange={e => setBorrower(e.target.value)}
                  placeholder="VD: Phòng Nguồn Vốn..."
                  className="w-full rounded-md border border-gray-300 p-2 text-xs bg-white focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
            </div>
          ...
          {/* Form phụ theo lý do Xuất bán / Sang tên cho khách */}
          {(selectedOpt?.reason === 'xuất bán' || selectedOpt?.reason === 'sang tên cho khách') && (
            <div className="grid grid-cols-2 gap-4 bg-orange-50/60 p-3.5 rounded-lg border border-orange-200">
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">
                  Cập nhật Trạng thái bán <span className="text-red-500">*</span>
                </label>
                <select
                  value={saleStatus}
                  onChange={e => setSaleStatus(e.target.value as any)}
                  className="w-full rounded-md border border-gray-300 p-2 text-xs bg-white focus:border-blue-500 focus:ring-blue-500 font-medium"
                >
                  <option value="sold">Đã bán (Khách hàng đã nhận sổ)</option>
                  <option value="ready_for_sale">Sẵn sàng bán</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">Giá trị giao dịch (Tùy chọn)</label>
                <input
                  type="number"
                  placeholder="VNĐ"
                  value={salePrice}
              ...
              <div className="bg-indigo-50/50 p-3 rounded-lg border border-indigo-100">
                ...
                <div className="flex gap-4">
                  ...
                  <label className="flex items-center text-xs font-medium cursor-pointer">
                    <input
                      type="radio"
                      checked={splitType === 'reissue'}
                      onChange={() => setSplitType('reissue')}
                      className="mr-1.5"
                    />
                    Cấp đổi/Cấp lại
                  </label>
              ...
              {splitType !== 'reissue' && (
                <div className="space-y-2">
                  <button
                    ...
                    className="text-xs text-blue-600 font-semibold flex items-center hover:text-blue-800 cursor-pointer"
                  >
